...
import { MenubarModule, MenuItem } from "@app/components/public_api";
...
export class EditInvoiceComponent implements OnInit, CanDeactivateComponent {
  ...
  invoicePaymentDetailsBasedType: string = "consultant";
  currencyIcon: string = "$";
  ...
  selectedIinvoiceRate: invoiceEntries;
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.selectedIinvoiceRate = new invoiceEntries();
    ...
    if (this.invoiceDetailsId) {
      ...
      console.log(this.selectedApproverEmailList);
    } else {
      this.invoicesEntries = [];
      this.invoicesDetails.organizationDetailsId = this.organizationDetailsId;
      let date = new Date();
  ...
  onCompanyList(event) {
    var creditDatefromList = {
      value: event.creditDays,
    };
    ...
    this.oncreditDays(creditDatefromList);
    const creditDays = this.creditDaysOptions.filter((element) => {
      if (element.value) {
        return element.value == event.creditDays;
      }
    });
    ...
    this.consultantOption.filter((res) => {
      if (res.value != null) {
        if (res.value.employeeDetailsId == event.value.employeeDetailsId) {
          ...
          this.invoicesEntries[index].overtimeExemption =
            res.value.overtimeExemption;
          res.value.rates.filter((res) => {
            if (res.rateType == "Rate") {
              this.invoicesEntries[index].rate = res.rate;
            }
          });
        }
      }
  ...
  async saveInvoice(invoicesDetails, saveDraft) {
    await this.submitInvoices(invoicesDetails, saveDraft).then((res) => {
      ...
      this.f.resetForm();
      this.router.navigate(["/admin/invoices"], { relativeTo: this.route });
    });
  }
  ...
  entryType(event, entryType) {
    console.log(this.invoicePaymentDetailsBasedType);
    this.invoicesEntries = [];
    this.onAddPaymentDetails();
    this.cdr.detectChanges();
    this.grandTotal = 0;
  ...
  fileUpload(form: NgForm) {
    return new Promise<void>((resolve, reject) => {
      let fd = new FormData();
      console.log(this.invoiceDetailsId.toString());
  ...
  onSelectInvoiceRate(rowData, index) {
    this.selectedIinvoiceRate = rowData;
    this.selectedIinvoiceRateIndex = index;
  }
}
