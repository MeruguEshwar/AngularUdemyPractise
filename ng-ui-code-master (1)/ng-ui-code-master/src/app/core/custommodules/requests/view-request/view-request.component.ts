...
export class ViewRequestComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this.state && this.state.requestDetails?.employeeRequestDetailsId) {
      this.employeeRequestDetailsId =
        this.state.requestDetails.employeeRequestDetailsId;
      this.empRequestDetails = await this.getEmpRequestDetails(
        this.employeeRequestDetailsId
      );
    }
  ...
  getEmpRequestDetails(employeeRequestDetailsId) {
  ...
  getMessages(employeeRequestDetailsId) {
    this.empRequestService
      ...
      .subscribe((res) => {
        if (res.statusCode == 200) {
          this.empRequestDetails.employeeRequestMessages = res.responsePayload;
        }
      });
  }
  sendMessage(details) {
    if (details.message) {
      if (!details.employeeRequestDetailsId) {
        details.employeeRequestDetailsId =
          this.empRequestDetails.employeeRequestDetailsId;
      }
      this.empRequestService
        .sendMessage(this.messageDetails)
        .subscribe((res) => {
          if (res.statusCode == 200) {
            this.getMessages(details.employeeRequestDetailsId);
            this.messageDetails = new EmployeeRequestMessages();
          }
        });
    }
  ...
  saveUploadFile(form: NgForm) {
    ...
    this.empRequestService.uploadEmpReqMsgFile(fd).subscribe(
      (res) => {
        if (res.statusCode == 200) {
          this.getMessages(this.empRequestDetails.employeeRequestDetailsId);
